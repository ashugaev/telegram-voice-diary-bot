...
- Chronology events are dated: `YYYY-MM-DD — what happened`. `roast.extract_chronology_events` is a separate model call per entry — same ops protocol, model, and budget as the profile — and gets `today`, so an entry that names no date still lands on the timeline. The roast system prompt always carries `Сегодня: <services/diary_dates.diary_today()>`, even when the chronology is empty; chronology sits above the rules.
...
- Every store announces its own note through the shared renderer `_render_memory_note`: `MEMORY_NOTE_HEADER` plus one block — `About you:` for profile facts, `Rules:` for rules, `Chronology:` for dated events — dropped when empty. Profile and chronology are separate passes, so an entry moving both sends two notes. Nothing changed sends no message. Long notes chunk through `_split_message`; a note failure never costs the stored change.
