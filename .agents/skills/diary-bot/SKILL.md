...
| Roast persona, author profile, behavior rules, chronology | `services/roast.py`, `tests/test_roast.py` |
...
- `/memory` rebuild is two-step (focus prompt, then confirm), sequential, single-flight, and persists points after every note. It replays saved notes into the profile only; chronology grows from new entries.
- The three memory stores (author profile, behavior rules, chronology) are accumulated `MemoryItem` lists edited only through `memory.apply_ops`: the model returns `create`/`modify`/`delete` operations addressing an item id, never a rewritten list. An unknown id is a logged no-op. Ids persist in state; list size and item length are guided in the prompt, never trimmed in code.
...
- Chronology events are dated: `YYYY-MM-DD — what happened`. Ingestion extracts profile facts and chronology events in a single model call (`roast.extract_memory`) returning separate ops lists, and gets `today` so an entry that names no date still lands on the timeline. The roast system prompt carries background extraction guidance and `Сегодня: <services/diary_dates.diary_today()>`, even when the chronology is empty; chronology sits above the rules.
...
- Every store announces its own note through the shared renderer `_render_memory_note`: `MEMORY_NOTE_HEADER` plus blocks — `About you:` for profile facts, `Rules:` for rules, `Chronology:` for dated events — dropped when empty. Ingestion updates profile and chronology together in one pass, sending a single consolidated note when both move. Nothing changed sends no message. Long notes chunk through `_split_message`; a note failure never costs the stored change.
- Replying to a memory note in any mode continues the roast conversation and schedules a contextual memory update pass; in Chat mode, replies update the chat conversation chain.
- Notion memory pages (`Memory — Author profile`, `Memory — Bot rules`, `Memory — Chronology`) sync both ways in plain text — ids stay internal. A page whose bullets no longer match the stored `notion_mirror` was hand-edited and wins; otherwise the page is rewritten from local state. Adopt through `memory.adopt`, which keeps the id of every bullet whose text survived the edit. Pull before every read of memory (roast, `/rules`, `/memory`, startup), push after every change, never block the diary flow. The roast pull is throttled by `MEMORY_PULL_TTL_SECONDS`; explicit reads always pull. Nothing polls in the background. One `_memory_lock` guards every memory read-modify-write — take it via `_sync_*_memory`, or call `_sync_*_memory_held` when already holding it; never hold it across a model call. The profile rebuild pulls once at the start and pushes once at the end, not per note.
