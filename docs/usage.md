...
3. The formatter generates a title, tags, and lightly cleaned text.
4. The bot shows the formatted preview. The original recognized text is stored in Notion as `Original Text`.
5. Press **Save** — the entry becomes its own row in your Notion database.
...
Every day at 21:00 (your timezone) the bot posts a summary of that day's entries, or a friendly reminder if there were none. Daily and weekly summaries include a small stats block (entry count, saved audio minutes, and the busiest day for weekly reports). Use `/weekly` to generate the weekly report on demand.
