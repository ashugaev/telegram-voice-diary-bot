...
def build_audio_stats_from_pages(pages: list[dict], today: date | None = None, language: str | None = None) -> AudioStats:
    ...
    for offset in range(STAT_DAYS):
        ...
        daily.append(_bucket(_date_label(current_day, lang), daily_records))
    ...
    for offset in range(STAT_MONTHS):
        ...
        monthly.append(_bucket(_month_label(current_month, lang), month_records))
    ...
    return AudioStats(
        total=_bucket(t("stats.all_time", lang), records),
        week=_bucket(t("stats.last_7_days", lang), week_records),
        daily=daily,
        monthly=monthly,
    )
...
def build_period_stats_from_pages(pages: list[dict], language: str | None = None) -> PeriodStats:
    ...
    if day_buckets:
        buckets = [
            _bucket(_date_label(day, lang), day_records)
...
def format_duration(seconds: int, language: str | None = None) -> str:
    ...
    if minutes == 0:
        return t("stats.duration.zero", lang)
    hours, minutes = divmod(minutes, 60)
    if hours and minutes:
        return t("stats.duration.hours_minutes", lang, hours=hours, minutes=f"{minutes:02d}")
    if hours:
        return t("stats.duration.hours", lang, hours=hours)
    return t("stats.duration.minutes", lang, minutes=minutes)


def _audio_label(value: int, language: str | None = None) -> str:
    lang = _stats_language(language)
    return t("stats.audio_count", lang, count=value)
...
def format_audio_stats(stats: AudioStats, language: str | None = None) -> str:
    lang = _stats_language(language)
    return "\n".join([
        t("stats.audio_header", lang),
        "",
        t(
            "stats.total_line",
            lang,
            duration=format_duration(stats.total.seconds, lang),
            count=_audio_label(stats.total.count, lang),
        ),
        t(
            "stats.week_line",
            lang,
            duration=format_duration(stats.week.seconds, lang),
            count=_audio_label(stats.week.count, lang),
        ),
        "",
        t("stats.daily_header", lang),
        *[_bucket_line(bucket, lang) for bucket in stats.daily],
        "",
        t("stats.monthly_header", lang),
        *[_bucket_line(bucket, lang) for bucket in stats.monthly],
    ])
...
def format_daily_stats(stats: PeriodStats, language: str | None = None) -> str:
    lang = _stats_language(language)
    return "\n".join([
        t("stats.day_header", lang),
        t("stats.entries_line", lang, count=stats.entry_count),
        t(
            "stats.audio_line",
            lang,
            duration=format_duration(stats.audio_seconds, lang),
            count=_audio_label(stats.voice_count, lang),
        ),
    ])
    ...
    if stats.busiest_day:
        lines.append(
            t(
                ...
                duration=format_duration(stats.busiest_day.seconds, lang),
            )
        )
    return "\n".join(lines)
