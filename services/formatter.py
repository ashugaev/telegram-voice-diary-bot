...
async def format_entry(transcription: str, language: str | None = None) -> tuple[str, str, list[str]]:
    ...
    if len(transcription) > LONG_TRANSCRIPTION_CHAR_LIMIT:
        system_prompt = _with_language(METADATA_PROMPTS[lang], lang) if language else METADATA_PROMPT
    ...
    system_prompt = _with_language(SYSTEM_PROMPTS[lang], lang) if language else SYSTEM_PROMPT
    response = await client.chat.completions.create(
        model=settings.formatter_model,
